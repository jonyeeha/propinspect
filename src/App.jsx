...
// Reads a file and corrects EXIF orientation so iPhone photos appear upright in PDFs
const toDataURL = f => new Promise(res => {
  const reader = new FileReader();
  reader.onload = e => {
    const dataUrl = e.target.result;
    // Read EXIF orientation from the raw bytes
    ...
    // Apply rotation on a canvas based on orientation value
    const fixOrientation = (orientation, dataUrl, cb) => {
      const img = new Image();
      img.onload = () => {
    ...
    // Read raw bytes to get EXIF, then fix and return
    const raw = new FileReader();
    raw.onload = ev => {
      const orientation = getOrientation(ev.target.result);
      if (orientation > 1) {
        fixOrientation(orientation, dataUrl, corrected => res(corrected));
      } else {
        res(dataUrl);
      }
    };
    raw.readAsArrayBuffer(f);
  };
...
export default function App() {
  ...
  const createWO = async () => {
    ...
    let data = null;
    await withRetry(async () => {
      const result = await supabase.from("work_orders").insert(wo).select().single();
  ...
  const updateWO = async (id, changes) => {
    // Optimistic update -- apply immediately, sync in background with retry
    setWorkOrders(prev=>prev.map(w=>w.id===id?{...w,...changes}:w));
    try {
      await withRetry(async () => {
        const { data, error } = await supabase.from("work_orders").update(changes).eq("id",id).select().single();
        if (error) throw error;
        if (data) setWorkOrders(prev=>prev.map(w=>w.id===id?data:w));
      });
    } catch(e) {
      console.error("updateWO failed:", e);
      alert("Could not update work order. Please check your connection and try again.");
    }
  };
  const deleteWO = async id => {
    // Optimistic delete -- remove immediately, confirm in background
    ...
    try {
      ...
      console.error("deleteWO failed:", e);
      // Restore the deleted item by reloading
      supabase.from("work_orders").select("*").eq("id",id).single()
        .then(({data}) => { if(data) setWorkOrders(prev=>[data,...prev]); });
      alert("Could not delete work order. Please try again.");
    }
  };
  const deleteInspection = async id => {
    ...
    try {
      ...
      alert("Could not delete inspection. Please try again.");
    }
  };
